...
for d_pair in sorted_matrix:
    ...
    if len(circuits) > 0:
        if len(circuits[0]) == len(input):
            print("all are joined in one circuit")
            break
...
print(f"last pair to be joined was {[shortest_distance[0], shortest_distance[1]]}")
print(f"result is {shortest_distance[0][0] * shortest_distance[1][0]}")